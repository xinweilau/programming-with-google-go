...
func main() {
    ...
    for scanner.Scan() {
        ...
        nameArr = append(nameArr, Name{ fname: split[0], lname: split[1] })
    }

    for idx, value := range nameArr {
        fmt.Printf("Person %d (fname = %s, lname = %s)\n", idx, value.fname, value.lname)
    }
